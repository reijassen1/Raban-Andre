...
export function MasterComparison() {
  ...
  const currentScriptObj = SCRIPTS[currentScriptIndex];
  ...
  }, [isRecording, scriptText, handleStopRecording]);
  ...
  return (
    ...
    >
      <header className="mb-8">
        <h2 className="text-3xl font-bold tracking-tight text-white mb-2">文本跟读评测</h2>
        <p className="text-neutral-400">发音与语态智能测评系统</p>
      </header>
        ...
        <div className="lg:col-span-7 flex flex-col gap-6 min-h-0">
          <div className="grid grid-cols-2 gap-4">
            ...
            <div className="relative rounded-2xl overflow-hidden bg-neutral-900 border border-neutral-800 aspect-video shadow-2xl">
              <div className="absolute top-4 left-4 bg-black/60 backdrop-blur-md px-3 py-1.5 rounded-full flex items-center gap-2 z-10">
                ...
                <span className="text-xs font-medium text-emerald-400">黄金标准库</span>
              </div>
              <img 
                src={`https://picsum.photos/seed/newsanchor${currentScriptIndex}/1280/720`} 
                alt="Master Anchor" 
                className="w-full h-full object-cover opacity-80"
                referrerPolicy="no-referrer"
              />
              <div className="absolute bottom-0 inset-x-0 bg-gradient-to-t from-black/80 to-transparent p-4">
                <div className="text-sm font-medium text-white mb-1">{currentScriptObj.title}</div>
              </div>
            </div>
          ...
          <div className="flex-1 bg-neutral-900/50 border border-neutral-800 rounded-2xl p-5 backdrop-blur-sm flex flex-col min-h-0">
            <h3 className="text-sm font-medium text-neutral-400 mb-4 uppercase tracking-wider flex items-center gap-2">
              ...
              对照文本：{currentScriptObj.title.split(' - ')[1]}
            </h3>
          ...
          <div className="flex-1 bg-neutral-950 rounded-2xl p-5 border border-neutral-800/50 mt-2">
            ...
            {score && (
              <div className="space-y-4">
                ...
                {currentScriptObj.feedbacks.map((fb, idx) => (
                  <div key={idx}>
                    <FeedbackItem 
                      type={fb.type} 
                      title={fb.title}
